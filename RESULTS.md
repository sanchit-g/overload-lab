...
Two values differ for the pool sweep in Section 1: Hikari max pool becomes the independent
variable (5 / 10 / 20 / 40), and the HTTP client pool is raised from 32 to **64** so that it
cannot bind before Hikari does. Every other parameter above is unchanged.
...
Sweeping the pool size supports the relationship, approximately, over an eightfold range:
...
Linear to within 5.6% — but this is not a clean confirmation: only c=5 demonstrably plateaued,
so for c=10, 20 and 40 the capacity figure is the top-step completion and is probably an
underestimate. (It is also why the sweep puts c=20 at 748 ev/s, slightly below the 754 ev/s
baseline figure above; they are separate runs and both measurements stand.) The 5.6% is
systematic rather than scatter and its cause is unresolved — see the notebook.
...
Medians over the 2x run below, excluding the first 30 s; all three 2x replicates agree.

```
hikaricp_connections_active    20     pegged at max
hikaricp_connections_pending   46     FLAT -- 64 workers less the 20 holding a connection
                                      accounts for 44 of these; the remainder are the
                                      health check's validation query and Tomcat request
                                      threads, which draw on the same pool
overload_http_pool_leased      20     never approaches its max of 32
...
2. Above that, the unbounded queue absorbs the excess at `offered - completed` per second: at
   2x, 1,480 - 719 = 761 against a measured queue slope of 763/s; at 4x, 2,960 - 586 = 2,374
   against 2,347/s. Both terms are in the Section 2 table.
...
**Saturation itself does not get worse.** `hikari_pending` pegs at 46 and stays there whether
you are at 2x or 4x — the widest range measured. An alert on `pending > 40` fires once and
then reports nothing about how bad things are getting.
...
*(At 2x the correspondence **does not fit** — ratio 0.60 to 0.99 depending on the baseline
chosen. The notebook explains why the baseline is soft there.)*

## 5. What this says about the design

The unbounded queue added **no capacity whatsoever** — completion was 740 ev/s at 1x, 719 at
